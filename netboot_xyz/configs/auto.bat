...
set SHARE=SAMBA-SHARE-NAME
set SETUP_PATH=PATH-TO-x64-SETUP-FOLDER
set USERNAME=SAMBASHARE-USERNAME
set AUTOATTEND_PATH=PATH-TO-AUTOATTEND.XML-FROM-SHARE-FOLDER
...
for /L %%i in (1,1,3) do (
    \\%SERVER%\%SHARE%\%SETUP_PATH%\setup.exe /unattend:\\%SERVER%\%SHARE%\%AUTOATTEND_PATH%
    if %errorlevel% == 0 (
